...
function Sale() {
    ...
    const handleAddToCart = (event, productId) => {
        ...
        if (quantity > item.qty) {
            setErrorMessage('Qty you entered exceeds the qty in stock');
            return;
        }
        ...
        <>
            ...
            <div>
                <div className={styles.productSearch}>
                    <input type="text" placeholder="Product Search" value={searchTerm} onChange={handleSearch} />
                </div>

                {filteredProducts.length > 0 && (
                    <div className={styles.searchList}>
                        {filteredProducts.map((filteredProduct, index) => (
                            <div key={index} onClick={() => handleClick(filteredProduct.product_id, filteredProduct.product_name)}>
                ...
                <table id="myTable">
                    <thead style={{ background: 'green' }}>
                        <tr>
                            ...
                            <th>Price</th>
                            <th>Unit</th>
                            <th className="hideDuringPrint">Action</th>
                        </tr>
                    </thead>
                    <tbody style={{ backgroundColor: 'gold' }}>
                        {addToCart.map((cart) => (
                            <tr key={cart.product_id}>
                                ...
                                <td>{cart.units}</td>
                                <td className="hideDuringPrint"></td>
                            </tr>
                        ))}
                        ...
                        <tr>
                            <td></td>
                            <td></td>
                            <td style={{ fontSize: '35px' }}>Grand Total:</td>
                            <td style={{ fontSize: '30px' }}>{calculateTotalPrice() !== 0 ? ` ${calculateTotalPrice()}` : ''}</td>
                            <td>
                                <button className={`${styles.button} hideDuringPrint`} onClick={handleCheckOut} style={{ background: 'green', color: 'white', borderRadius: '25px' }}>
                                    Buy Now
                                </button>
                            </td>
                            <td>
                                <button className="hideDuringPrint" onClick={handlePrintTable}>
                                    Print
                                </button>
                            </td>
                        </tr>
